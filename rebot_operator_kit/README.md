...
5. Record a new five-episode smoke dataset from the training page. This uses
...
7. After the smoke set passes, create a new production dataset with 50–100
   clean successful episodes. Train MolmoAct2 on an NVIDIA GPU host using the
   command generated by the GUI; do not attempt MolmoAct2 training on this Mac.

## Demonstration technique

- Use one plain-language task string for the entire dataset: `Pick up the
  crumpled paper ball and place it in the trash bin`.
...
- After every kept or failed episode, the follower automatically returns to its
  ...
  reset the object/bin; the next attempt is blocked until both arms are aligned
  and the reset timer has completed. Reset motion is not recorded.
- Move smoothly and decisively. Avoid repeated corrections, collisions,
  hesitations, and covering the object with your hand or body.
...
- The first five episodes are only a pipeline smoke test. After they pass,
  create a fresh production dataset with at least 50 clean successful episodes,
  including at least ten examples for each planned object-location variant.
...
camera. Five episodes are only a recording/schema smoke test. A first useful
run should use 50–100 clean episodes and the GUI's native 7D MolmoAct2 recipe
on an NVIDIA GPU.
